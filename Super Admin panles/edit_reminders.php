...
if (isset($_GET['id'])) {
    $newsId = $_GET['id'];
    $sql = "SELECT * from tbl_reminder WHERE id = $newsId";
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content rounded-4 shadow">
                    <div class="modal-header p-5 pb-4 border-bottom-0">
                        <h2 class="fw-bold mb-4 fs-2">Edit Remainder Details</h2>
                    </div>

                    <div class="modal-body p-5 pt-0">
                        <form action="edit_reminders.php" method="post" enctype="multipart/form-data">
                            ...
                            <div class="col-sm-12">
                                <label for="image">Remainder Thumbnail</label>
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['image']['name'])) {
        ...
        if ($image_name != "") {
            ...
            $image_name = "EDIT_Remainder" . rand(0000, 9999) . '.' . $ext;
    ...
    // Update the reminder details in the database using prepared statements
    $updateQuery = "UPDATE tbl_reminder SET image = ?, title = ?, date = ?, discription = ?, view = ?, LastupdateAt = ? WHERE id = ?";

    $lastUpdateAt = date('Y-m-d H:i:s'); // Current timestamp for LastupdateAt

    $stmt = $conn->prepare($updateQuery);
    $stmt->bind_param("ssssssi", $image_name, $new_title, $new_date, $new_description, $new_view, $lastUpdateAt, $new_id);

    if ($stmt->execute()) {
            // Redirect to the reminders dashboard or display a success message
