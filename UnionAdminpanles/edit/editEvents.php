...
if (isset($_GET['id'])) {
    $newsId = $_GET['id'];
    $sql = "SELECT * from tbl_event WHERE id = $newsId";
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content rounded-4 shadow">
                    <div class="modal-header p-5 pb-4 border-bottom-0">
                        <h2 class="fw-bold mb-4 fs-2">Edit Event Details</h2>
                    </div>
...
if (isset($_POST['submit'])) {
    ...
    $updateQuery = "UPDATE tbl_event SET image = '$image_name', titile = '$new_title', date = '$new_date', description = '$new_description', view = '$new_view' WHERE id = $new_id";

    if ($conn->query($updateQuery) === TRUE) {
        // Redirect to the news dashboard or display a success message
        header("Location: Events.php?id=".$new_id);
        exit();
    } else {
